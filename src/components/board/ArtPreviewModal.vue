...
import { ref } from 'vue'
import { X, Eye, XCircle, Send, Calendar, MessageSquare, Check, Clock, Info, Maximize } from 'lucide-vue-next'
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-300">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
        <div class="flex items-center gap-3">
          <div :class="['w-10 h-10 rounded-xl flex items-center justify-center shadow-sm', mode === 'internal-review' ? 'bg-indigo-100 text-indigo-600' : 'bg-primary/10 text-primary']">
            <Eye v-if="mode === 'preview'" class="w-5 h-5" />
            <Check v-else class="w-5 h-5" />
          </div>
          <div>
            <h2 class="text-xl font-bold text-gray-900">
              {{ mode === 'internal-review' ? 'Aprovação Interna' : (approval?.artName === 'Referencia.jpg' ? 'Ver Referência' : 'Visualizar Post') }}
            </h2>
      ...
      <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-4">
          <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Mídia do Conteúdo</label>
          <div class="relative aspect-square rounded-[2rem] border border-gray-100 bg-gray-950 flex items-center justify-center overflow-hidden shadow-2xl group">
        ...
        <div class="space-y-5 flex flex-col">
          ...
          <div class="flex items-center justify-between px-1">
             <div class="flex flex-col">
                <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Cliente</span>
             ...
             <div v-if="approval?.status" class="flex flex-col items-end">
                ...
                <span :class="['text-[10px] font-bold px-2 py-0.5 rounded-lg border', 
                  approval.status === 'APPROVE' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-amber-50 border-amber-100 text-amber-600']">
          ...
          <div class="flex-1 space-y-2">
            ...
            <div class="w-full p-5 rounded-[1.5rem] border border-gray-100 bg-gray-50/50 text-sm leading-relaxed text-gray-600 max-h-[220px] overflow-y-auto whitespace-pre-line shadow-inner">
              {{ approval?.caption || 'Sem legenda cadastrada.' }}
            </div>
          </div>

          <!-- Agendamento e Notas -->
          <div v-if="mode === 'internal-review'" class="space-y-4 animate-in slide-in-from-bottom-2 duration-300">
             <div class="grid grid-cols-1 gap-4">
                <div>
                   <label class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest block mb-1.5 ml-1">Data de Postagem</label>
                   <input
                      type="datetime-local"
                      :value="scheduledAt"
                ...
                <div>
                   <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1.5 ml-1">Notas de Revisão</label>
                   <textarea
          ...
          <!-- Histórico de Envio -->
          <div v-if="approval?.whatsappSentAt" class="p-3 rounded-xl border border-emerald-100 bg-emerald-50/30 flex items-center gap-3">
             <div class="w-7 h-7 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-600">
                <Send class="w-3.5 h-3.5" />
             </div>
             <span class="text-[10px] font-bold text-emerald-700">Enviado ao cliente em {{ formatSentAt(approval.whatsappSentAt) }}</span>
          </div>
      ...
      <div class="p-6 bg-gray-50 border-t border-gray-100 flex gap-4">
        <Button variant="outline" class="flex-1 h-12 rounded-xl font-bold border-gray-200" @click="$emit('close')">
        ...
        <template v-if="mode === 'internal-review'">
          ...
          >
            ...
            <template v-else>
              <Send class="w-4 h-4" /> ENVIAR AO CLIENTE
            </template>
          </Button>
        </template>
      </div>
    ...
    <!-- Reject Reason Modal -->
    <div v-if="isRejectReasonModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-red-50 text-red-500 flex items-center justify-center"><XCircle class="w-5 h-5" /></div>
            <h3 class="text-lg font-bold text-gray-900">Motivo da Rejeição</h3>
          </div>
          <button @click="$emit('closeRejectModal')" class="p-2 hover:bg-gray-100 rounded-xl text-gray-400"><X class="w-5 h-5" /></button>
        </div>
        <div class="p-7">
          <textarea
          ...
          ></textarea>
          <p v-if="rejectionReasonError" class="mt-2 text-[10px] font-bold text-red-500 uppercase tracking-widest">{{ rejectionReasonError }}</p>
        </div>
        <div class="p-6 bg-gray-50 border-t border-gray-100 flex gap-3">
          <Button variant="outline" class="flex-1 h-12 rounded-xl font-bold" @click="$emit('closeRejectModal')">CANCELAR</Button>
          <Button variant="destructive" class="flex-1 h-12 rounded-xl font-bold" :disabled="internalReviewAction === 'reject'" @click="$emit('confirmRejection')">
            CONFIRMAR
          </Button>
        </div>
      </div>
