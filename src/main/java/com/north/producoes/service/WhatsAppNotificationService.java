...
public class WhatsAppNotificationService {
    ...
    public void sendApprovalRequest(Long clientId, Long postId, Long approveId, List<String> mediaUrls) {
        ...
        try {
            // Passo 1: envia a(s) imagem(ns), com a legenda acoplada na última mídia
            for (int i = 0; i < mediaUrls.size(); i++) {
                boolean isLast = i == mediaUrls.size() - 1;
                String caption = isLast ? buildImageCaption(post, approve) : null;
                evolutionApiClient.sendMediaToGroup(groupId, mediaUrls.get(i), caption);
            }
